...
public class OrderInfoController extends BaseController {
	...
	@InitBinder("placeObj")
	public void initBinderplaceObj(WebDataBinder binder) {
		binder.setFieldDefaultPrefix("placeObj.");
	}
	...
	public void list(@ModelAttribute("userObj") UserInfo userObj,@ModelAttribute("placeObj") Place placeObj,String orderDate,@ModelAttribute("intervalObj") TimeInterval intervalObj,@ModelAttribute("payWayObj") PayWay payWayObj,String payAccount,String orderStateObj,String orderTime,String receiveName,String telephone,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		...
		List<OrderInfo> orderInfoList = orderInfoService.queryOrderInfo(userObj, placeObj, orderDate, intervalObj, payWayObj, payAccount, orderStateObj, orderTime, receiveName, telephone, page);
	    /*计算总的页数和总的记录数*/
	    orderInfoService.queryTotalPageAndRecordNumber(userObj, placeObj, orderDate, intervalObj, payWayObj, payAccount, orderStateObj, orderTime, receiveName, telephone);
	    /*获取到总的页码数目*/
	...
	public String frontlist(@ModelAttribute("userObj") UserInfo userObj,@ModelAttribute("placeObj") Place placeObj,String orderDate,@ModelAttribute("intervalObj") TimeInterval intervalObj,@ModelAttribute("payWayObj") PayWay payWayObj,String payAccount,String orderStateObj,String orderTime,String receiveName,String telephone,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		...
		if (receiveName == null) receiveName = "";
		if (telephone == null) telephone = "";
		List<OrderInfo> orderInfoList = orderInfoService.queryOrderInfo(userObj, placeObj, orderDate, intervalObj, payWayObj, payAccount, orderStateObj, orderTime, receiveName, telephone, currentPage);
	    /*计算总的页数和总的记录数*/
	    orderInfoService.queryTotalPageAndRecordNumber(userObj, placeObj, orderDate, intervalObj, payWayObj, payAccount, orderStateObj, orderTime, receiveName, telephone);
	    /*获取到总的页码数目*/
	    int totalPage = orderInfoService.getTotalPage();
	    ...
	    request.setAttribute("placeObj", placeObj);
	...
	public String delete(@PathVariable Integer orderNo,HttpServletRequest request) throws UnsupportedEncodingException {
		  try {
			  orderInfoService.deleteOrderInfo(orderNo);
	            request.setAttribute("message", "预约订单删除成功!");
	            return "message";
	        } catch (Exception e) { 
	            e.printStackTrace();
	            request.setAttribute("error", "预约订单删除失败!");
				return "error";
	...
	public void OutToExcel(@ModelAttribute("userObj") UserInfo userObj,@ModelAttribute("placeObj") Place placeObj,String orderDate,@ModelAttribute("intervalObj") TimeInterval intervalObj,@ModelAttribute("payWayObj") PayWay payWayObj,String payAccount,String orderStateObj,String orderTime,String receiveName,String telephone, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        ...
        if(receiveName == null) receiveName = "";
        if(telephone == null) telephone = "";
        List<OrderInfo> orderInfoList = orderInfoService.queryOrderInfo(userObj,placeObj,orderDate,intervalObj,payWayObj,payAccount,orderStateObj,orderTime,receiveName,telephone);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "OrderInfo信息记录"; 
        String[] headers = { "订单编号","下单用户","预约会议室","预定日期","预约时段","订单总金额","支付方式","支付账号","订单状态","下单时间","收货人","收货人电话"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<orderInfoList.size();i++) {
        	OrderInfo orderInfo = orderInfoList.get(i); 
        	dataset.add(new String[]{orderInfo.getOrderNo() + "",orderInfo.getUserObj().getName(),orderInfo.getPlaceObj().getPlaceName(),orderInfo.getOrderDate(),orderInfo.getIntervalObj().getIntervalName(),orderInfo.getTotalMoney() + "",orderInfo.getPayWayObj().getPayWayName(),orderInfo.getPayAccount(),orderInfo.getOrderStateObj(),orderInfo.getOrderTime(),orderInfo.getReceiveName(),orderInfo.getTelephone()});
        }
        /*
