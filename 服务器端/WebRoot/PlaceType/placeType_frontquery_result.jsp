...
<%
    ...
    List<PlaceType> placeTypeList = (List<PlaceType>)request.getAttribute("placeTypeList");
...
<jsp:include page="../header.jsp"></jsp:include>
		...
		<div class="col-md-12 wow fadeInDown" data-wow-duration="0.5s">
			<div>
				...
				<ul class="nav nav-tabs" role="tablist">
			    	...
			    	<li role="presentation" ><a href="<%=basePath %>PlaceType/placeType_frontAdd.jsp" style="display:none;">添加会议室类型</a></li>
				</ul>
			  	<!-- Tab panes -->
				    				...
				    				<table class="table table-condensed table-hover">
				    					...
				    					<% 
				    	            		...
				    	            		for(int i=0;i<placeTypeList.size();i++) {
					    	            		...
					    	            		PlaceType placeType = placeTypeList.get(i); //获取到会议室类型对象
 										%>
 										<tr>
 											<td><%=currentIndex %></td>
 											<td><%=placeType.getPlaceTypeId() %></td>
 											<td><%=placeType.getPlaceTypeName() %></td>
 											<td>
 												<a href="<%=basePath  %>PlaceType/<%=placeType.getPlaceTypeId() %>/frontshow"><i class="fa fa-info"></i>&nbsp;查看</a>&nbsp;
 												<a href="#" onclick="placeTypeEdit('<%=placeType.getPlaceTypeId() %>');" style="display:none;"><i class="fa fa-pencil fa-fw"></i>编辑</a>&nbsp;
 												<a href="#" onclick="placeTypeDelete('<%=placeType.getPlaceTypeId() %>');" style="display:none;"><i class="fa fa-trash-o fa-fw"></i>删除</a>
 											</td> 
 										</tr>
 										<%}%>
				    				</table>
	...
	<div style="display:none;">
		...
		<form name="placeTypeQueryForm" id="placeTypeQueryForm" action="<%=basePath %>PlaceType/frontlist" class="mar_t15" method="post">
...
function placeTypeDelete(placeTypeId) {
	if(confirm("确认删除这个记录")) {
		$.ajax({
			type : "POST",
			url : basePath + "PlaceType/deletes",
			data : {
