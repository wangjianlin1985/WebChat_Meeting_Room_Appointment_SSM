...
Page({
  ...
  // 加载会议室类型列表
  listPlaceType: function () {
    var self = this
    var url = config.basePath + "api/placeType/list"
    //如果要显示的页码超过总页码不操作
    ...
    utils.sendRequest(url, {
      ...
      setTimeout(function () {  
        self.setData({
          placeTypes: self.data.placeTypes.concat(res.data.list),
  ...
  //删除会议室类型记录
  removePlaceType: function (e) {
    var self = this
    var placeTypeId = e.currentTarget.dataset.placetypeid
    wx.showModal({
      title: '提示',
      content: '确定要删除placeTypeId=' + placeTypeId + '的记录吗？',
      success: function (sm) {
        if (sm.confirm) {
          // 用户点击了确定 可以调用删除方法了
          var url = config.basePath + "api/placeType/delete/" + placeTypeId
          utils.sendRequest(url, null, function (res) {
            ...
            //删除会议室类型后客户端同步删除数据
            var placeTypes = self.data.placeTypes;
            for (var i = 0; i < placeTypes.length; i++) {
              if (placeTypes[i].placeTypeId == placeTypeId) {
                placeTypes.splice(i, 1)
                break
              }
            }
            self.setData({ placeTypes: placeTypes })
          })
        } else if (sm.cancel) {
          console.log('用户点击取消')
        }
